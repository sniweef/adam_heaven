...
class ArticleTypeSetting(db.Model):
    __tablename__ = 'article_type_settings'
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(64), unique=True)
...
class ArticleType(db.Model):
    ...
    articles = db.relationship('Article', backref='article_type', lazy='dynamic')
    menu_id = db.Column(db.Integer, db.ForeignKey('menus.id'), default=None)
    setting_id = db.Column(db.Integer, db.ForeignKey('article_type_settings.id'))
    ...
    def is_hide(self):
        if self.setting:
            ...
            return False
    # if the article_type does not have setting,
...
class Article(db.Model):
    ...
    def generate_fake(count=100):
        ...
        seed()
        article_type_count = ArticleType.query.count()
        source_count = Source.query.count()
        for i in range(count):
            aT = ArticleType.query.offset(randint(0, article_type_count - 1)).first()
