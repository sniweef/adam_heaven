...
@manage_blog_bp.route('/manage-articles', methods=['GET', 'POST'])
@login_required
def manage_articles():
    ...
    return render_template('manage/manage_articles.html', Article=Article,
                           articles=articles, pagination=pagination,
                           endpoint='blog.manage_articles',
                           form=form, form2=form2, form3=from3,
                           types_id=types_id, source_id=source_id, page=page)


@manage_blog_bp.route('/manage-articles/delete-article', methods=['GET', 'POST'])
@login_required
def delete_article():
    types_id = request.args.get('types_id', -1, type=int)
    source_id = request.args.get('source_id', -1, type=int)
    ...
    return redirect(url_for('blog.manage_articles', types_id=types_id, source_id=source_id,
                            page=request.args.get('page', 1, type=int)))


@manage_blog_bp.route('/manage-articles/delete-articles', methods=['GET', 'POST'])
@login_required
def delete_articles():
    types_id = request.args.get('types_id', -1, type=int)
    source_id = request.args.get('source_id', -1, type=int)
...
@manage_blog_bp.route('/manage-comments/disable/<int:id>')
@login_required
...
@manage_blog_bp.route('/manage-comments/enable/<int:id>')
@login_required
...
# 前面在manage页面删除单篇博文时使用表单而不是Ajax，其实使用Ajax效果会更好，当然这里只是尽可能
# 使用不同的技术，因为以后在做自动化运维开发时总有用得上的地方
@manage_blog_bp.route('/manage-comments/delete-comment/<int:id>')
@login_required
...
@manage_blog_bp.route('/manage-comments', methods=['GET', 'POST'])
@login_required
def manage_comments():
    ...
    return render_template('manage/manage_comments.html', User=User,
                           Comment=Comment, comments=comments,
                           pagination=pagination, page=page,
                           endpoint='.manage_comments', form=form, form2=form2)


@manage_blog_bp.route('/manage-comments/delete-comments', methods=['GET', 'POST'])
@login_required
def delete_comments():
    form2 = DeleteCommentsForm(commentIds=-1)
...
@manage_blog_bp.route('/custom/blog-info', methods=['GET', 'POST'])
@login_required
...
@manage_blog_bp.route('/custom/blog-plugin', methods=['GET', 'POST'])
@login_required
...
def delete_plugin(id):
    ...
    return redirect(url_for('blog.custom_blog_plugin', page=page))


@manage_blog_bp.route('/custom/blog-plugin/sort-up/<int:id>')
@login_required
...
@manage_blog_bp.route('/custom/blog-plugin/sort-down/<int:id>')
@login_required
...
@manage_blog_bp.route('/custom/blog-plugin/add', methods=['GET', 'POST'])
@login_required
def add_plugin():
...
def edit_plugin(id):
    ...
    if form.validate_on_submit():
        ...
        if plugin_check and plugin_check.id != id:
            ...
            return redirect(url_for('blog.edit_plugin', id=id))
        else:
            plugin.title = title
    ...
    return render_template('manage/blog_plugin_add.html', form=form, page=page)


@manage_blog_bp.route('/account/')
@login_required
def account():
...
@manage_blog_bp.route('/account/change-password', methods=['GET', 'POST'])
@login_required
def change_password():
    form = ChangePasswordForm()
